...
export default function VideoArea() {
  ...
  const videoLiked =
    video.active && userData?.activity
      ? userData.activity.likes.includes(video.element._id)
      : null;
  const videoDisliked =
    video.active && userData?.activity
      ? userData.activity.dislikes.includes(video.element._id)
      : null;
  ...
  const goToVideoAuthorsPage = () => {
    window.history.pushState(
      "search",
      "",
      process.env.REACT_APP_YOUPIPE_URI +
        "?search=" +
        encodeURI(video.element.author)
    );
    ChangeQuery({
  ...
  const handleLikePress = () => {
    userData
      ? updateUserData({
          ...
          UpdateFields: { activity: { likes: video.element._id } },
  ...
  const handleDislikePress = () => {
    userData
      ? updateUserData({
          AccessToken: userData.accessToken,
          UpdateFields: { activity: { dislikes: video.element._id } },
          ChangeUser: ChangeUser,
        })
      : setNoUserTrigger((prev) => !prev);
  };
  ...
  const handleSubscribePress = () => {
    userData
      ? updateUserData({
          AccessToken: userData.accessToken,
          UpdateFields: { activity: { subscriptions: video.element.author } },
          ChangeUser: ChangeUser,
        })
      : setNoUserTrigger((prev) => !prev);
  };
            ...
            <div className="videoArea-channelInfo">
              ...
              <div className="videoArea-ratingSection">
                ...
                <div
                  ...
                  title={
                    videoDisliked
                      ? video.element.rating.dislikes + 1
                      : video.element.rating.dislikes
                  }
