...
export default function MainArea() {
  ...
  const columnsNumber = useGridCalculator(
    mainContainerRef,
    320,
    ChangeSideMenu
  );
  ...
  const { loading, data, hasMore } = useFetchWithPagination({
    query: pureQuery,
  ...
  const paginationMarkerElementRef = useCallback(
    (node) => {
      if (loading) return;
      ...
      observer.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && hasMore > 0) {
          ChangeQuery({
            ...query,
            amountToFind: query.amountToFind + query.defaults.increment,
          });
        }
      });
  ...
  const MainAreaRowsFill = (props) => {
    ...
    const videoFeed = videoModeActive
      ? videos.filter((item) => item._id !== videoModeActive)
      : videos;
      ...
      if (notLastLine) {
        return (
          <RowContainer
            ...
            rowElementsNumber={rowLength}
            elements={videoFeed.slice(rowStart, rowEnd)}
          />
        );
      } else {
        lastLinePassed = true;
        return (
          <React.Fragment key={"PaginatorFragment" + index}>
            {hasMore > 0 && (
              <div
                className="paginationMarker"
                ref={paginationMarkerElementRef}
              ></div>
            )}
            <RowContainer
              key={"RowContainer" + index}
              rowElementsNumber={rowLength}
              elements={videoFeed.slice(rowStart, rowStart + rowLength)}
            />
          </React.Fragment>
        );
      }
  ...
  return (
    <div
      ...
      sidemenustate={
        video.active ? "2" : sideMenuOptions.sideMenuFolded ? "0" : "1"
      }
    ...
    >
      ...
      <MainAreaRowsFill
        videos={data}
        rowLength={columnsNumber}
        videoModeActive={video.element?._id}
      />
